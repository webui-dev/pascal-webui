...
function webui_bind(window: size_t; const element: PChar; func: TWebuiEventProc): size_t; stdcall; external{$ifndef STATICLINK} webuilib{$endif};
// Show a window using a embedded HTML, or a file. If the window is already opened then it will be refreshed.
function webui_show(window: size_t; const content: PChar): Boolean; stdcall; external{$ifndef STATICLINK} webuilib{$endif};
// Same as webui_show(). But with a specific web browser.
function webui_show_browser(window: size_t; const content: PChar; browser: size_t): Boolean; stdcall; external{$ifndef STATICLINK} webuilib{$endif};
...
function webui_set_root_folder(window: size_t; const path: PChar): Boolean; stdcall; external{$ifndef STATICLINK} webuilib{$endif};
// Set the web-server root folder path for all windows. Should be used before `webui_show()`.
function webui_set_default_root_folder(const path: PChar): Boolean; stdcall; external{$ifndef STATICLINK} webuilib{$endif};
...
procedure webui_set_icon(window: size_t; const icon, icon_type: PChar); stdcall; external{$ifndef STATICLINK} webuilib{$endif};
// Base64 encoding. Use this to safely send text based data to the UI.
function webui_encode(const str: PChar): PChar; stdcall; external{$ifndef STATICLINK} webuilib{$endif};
// Base64 decoding. Use this to safely decode received Base64 text from the UI.
function webui_decode(const str: PChar): PChar; stdcall; external{$ifndef STATICLINK} webuilib{$endif};
...
procedure webui_send_raw(window: size_t; const func: PChar; raw: Pointer; size: size_t); stdcall; external{$ifndef STATICLINK} webuilib{$endif};
...
procedure webui_set_profile(window: size_t; const name, path: PChar); stdcall; external{$ifndef STATICLINK} webuilib{$endif};
// Get the full current URL
function webui_get_url(window: size_t): PChar; stdcall; external{$ifndef STATICLINK} webuilib{$endif};
// Navigate to a specific URL
procedure webui_navigate(window: size_t; url: PChar); stdcall; external{$ifndef STATICLINK} webuilib{$endif};
...
procedure webui_run(window: size_t; const script: PChar); stdcall; external{$ifndef STATICLINK} webuilib{$endif};
// Run a JavaScript, and get the response back (Make sure your local buffer can hold the response).
function webui_script(window: size_t; const script: PChar; timeout: size_t; buffer: PChar; buffer_length: size_t): Boolean; stdcall; external{$ifndef STATICLINK} webuilib{$endif};
...
function webui_get_string_at(e: PWebUIEvent; index: size_t): PChar; stdcall; external{$ifndef STATICLINK} webuilib{$endif};
// Get the first argument as string
function webui_get_string(e: PWebUIEvent): PChar; stdcall; external{$ifndef STATICLINK} webuilib{$endif};
...
procedure webui_return_string(e: PWebUIEvent; const s: PChar); stdcall; external{$ifndef STATICLINK} webuilib{$endif};
...
// Bind a specific html element click event with a function. Empty element means all events.
function webui_interface_bind(window: size_t; const element: PChar; func: TWebUIInterfaceEventProc): size_t; stdcall; external{$ifndef STATICLINK} webuilib{$endif};
// When using `webui_interface_bind()` you may need this function to easily set your callback response.
procedure webui_interface_set_response(window, event_number: size_t; const response: PChar); stdcall; external{$ifndef STATICLINK} webuilib{$endif};
...
// Get an argument as string at a specific index
function webui_interface_get_string_at(window, event_number, index: size_t): Pchar; stdcall; external{$ifndef STATICLINK} webuilib{$endif};
// Get an argument as integer at a specific index
